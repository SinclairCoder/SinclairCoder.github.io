...
<br>
思路：
哈希表的应用，根据名字生成索引，然后将课程号记录。不过要注意，需要使用scanf输入，不能使用cin，否则会超时。
...
#define N 102
int hashfun(char s[5]){
	return (s[3]-'0')+(s[2]-'A')*10+(s[1]-'A')*260+(s[0]-'A')*26*260;
}
vector<int> course[26*26*26*11];
int main(){
	int n,k,index,cnt;
	cin >> n >> k;
	char name[5];
...
//		cin >> name;
		int h = hashfun(name);
		cout << name << " " << course[h].size();
		if(course[h].size()) {
			// 谁能想到还要排序.. 
			sort(course[h].begin(),course[h].end());
			for(int j=0;j<course[h].size();j++){
				cout << " " << course[h][j];
			}
		}
		cout << endl;
